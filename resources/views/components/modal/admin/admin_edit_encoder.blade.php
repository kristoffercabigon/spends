@if (session('clearAdminEditEncoderModal'))
<script>
    localStorage.removeItem('showAdminEditEncoderModal', 'true');
</script>
@endif

<div style="display: none" id="adminEditEncoderModal" class="fixed inset-0 bg-black bg-opacity-50 z-10 flex items-center justify-center font-poppins"
    x-show="showAdminEditEncoderModal"
    x-transition:enter="transition-opacity ease-linear duration-300"
    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
    x-transition:leave="transition-opacity ease-linear duration-300"
    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
    @click.away="showAdminEditEncoderModal = false; localStorage.setItem('showAdminEditEncoderModal', 'false')">
    <div @click.stop>
        <section class="bg-gray-50 relative mx-4 rounded-lg">
            <button @click="showAdminEditEncoderModal = false; localStorage.setItem('showAdminEditEncoderModal', 'false')" 
            ...
            <div class="w-[400px] bg-white rounded-lg shadow sm:max-w-md xl:p-0">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form x-data="{ isEncoderLoadingSignUp: false }"
                          ...
                          $nextTick(() => $el.submit());" 
                          class="space-y-4 md:space-y-6" method="POST" enctype="multipart/form-data" action="{{ route('admin-submit-edit-encoder', $encoder->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div id="content1" class="space-y-4 md:space-y-6 overflow-x-hidden overflow-y-auto max-h-80 px-2">
                            <div>
                                ...
                                <input type="text" name="encoder_first_name" id="encoder_first_name" placeholder="First Name"
                                    ...
                                    value="{{ $encoder->encoder_first_name }}">
                            ...
                            <div>
                                ...
                                <input type="text" name="encoder_suffix" id="encoder_suffix" placeholder="Enter suffix (e.g., Jr., Sr., III)"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" value="{{ $encoder->encoder_suffix }}">
