<div x-show="showAdminEncoderCameraModal" style="display: none" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center font-poppins"
    x-transition:enter="transition-opacity ease-linear duration-300"
     x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
     x-transition:leave="transition-opacity ease-linear duration-300"
     x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
    @click.away="showAdminEncoderCameraModal = false"
    @open-admin-encoder-camera-modal.window="initWebcam()">
    <div @click.stop class="bg-white mx-4 rounded-lg overflow-hidden shadow-xl transform transition-all max-w-[715px] w-full">
        <section class="bg-gray-50 relative">
            <button type="button" @click="showAdminEncoderCameraModal = false" class="absolute top-4 right-4 text-gray-500 hover:text-gray-700 focus:outline-none">
    ...
    function useEncoderCapturedPhoto() {
        ...
        fetch(imageData)
            ...
            .then(blob => {
                ...
                document.dispatchEvent(new CustomEvent('close-admin-encoder-camera-modal', { bubbles: true }));
            });
    }

    document.addEventListener('close-admin-encoder-camera-modal', function() {
        Alpine.store('showAdminEncoderCameraModal', false); 
    });
</script>
