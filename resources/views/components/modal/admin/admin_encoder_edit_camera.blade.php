<div x-show="showAdminEncoderEditCameraModal" style="display: none" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center font-poppins"
    x-transition:enter="transition-opacity ease-linear duration-300"
     x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
     x-transition:leave="transition-opacity ease-linear duration-300"
     x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
    @click.away="showAdminEncoderEditCameraModal = false"
    @open-admin-encoder-edit-camera-modal.window="initWebcam()">
    <div @click.stop class="bg-white mx-4 rounded-lg overflow-hidden shadow-xl transform transition-all max-w-[715px] w-full">
        <section class="bg-gray-50 relative">
            <button type="button" @click="showAdminEncoderEditCameraModal = false" class="absolute top-4 right-4 text-gray-500 hover:text-gray-700 focus:outline-none">
            ...
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8" style="max-height: 90vh; overflow-y: auto;">
                ...
                <div id="encoder_image_results" class="col-md-6 text-center text-gray-900">(Your captured image will appear here)</div>
                <div class="col-md-12 text-center">
                    ...
                    <button class="py-3 px-6 w-full md:w-auto text-sm tracking-wider font-light rounded-md text-white bg-blue-500 hover:bg-blue-600 focus:outline-none" type="button" onclick="useEncoderCapturedPhoto()">Use this photo</button>
                    </div>
                </div>
            </div>
        </section>
...
<script>
    ...
    function initWebcam() {
        ...
        const cameraElement = document.getElementById('my_encoder_camera');
    ...
    function take_snapshot() {
        Webcam.snap(function(data_uri) {
            ...
            document.getElementById('encoder_image_results').innerHTML = '<img src="'+data_uri+'"/>';
            document.getElementById('error-message').classList.add('hidden');
        });
    }

    function useEncoderCapturedPhoto() {
        ...
        const EncoderProfilePictureField = document.getElementById('EncoderProfilePictureField');
        ...
        fetch(imageData)
            ...
            .then(blob => {
                ...
                EncoderProfilePictureField.files = dataTransfer.files;

                EncoderProfilePictureField.dispatchEvent(new Event('change'));

                document.dispatchEvent(new CustomEvent('close-admin-encoder-edit-camera-modal', { bubbles: true }));
            });
    }

    document.addEventListener('close-admin-encoder-edit-camera-modal', function() {
        Alpine.store('showAdminEncoderEditCameraModal', false); 
    });
</script>
