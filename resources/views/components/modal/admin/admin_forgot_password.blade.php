@if (session('clearAdminForgotPasswordModal'))
<script>
    localStorage.removeItem('showAdminForgotPasswordModal');
</script>
@endif
...
<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-30 font-poppins"
    ...
    <div @click.stop>
        <section class="bg-gray-50 dark:bg-gray-900 relative">
            <button @click="showAdminForgotPasswordModal = false; localStorage.setItem('showAdminForgotPasswordModal', 'false')" 
            ...
            <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form x-data="{ isAdminLoadingForgotPassword: false }"
                        @submit.prevent="
                            isAdminLoadingForgotPassword = true;
                            $nextTick(() => $el.submit());
                        "
                        class="space-y-4 md:space-y-6" method="POST" action="{{ route('admin-forgot-password') }}">
                        @method('PUT')
                        @csrf
                        ...
                        <button type="submit" class="relative hover:animate-pop w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                            <span x-show="!isAdminLoadingForgotPassword">Send Reset Link</span>
                            <span x-show="isAdminLoadingForgotPassword" style="display: none;" class="flex items-center justify-center">
