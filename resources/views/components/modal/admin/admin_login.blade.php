@if (session('clearAdminLoginModal'))
<script>
    localStorage.removeItem('showAdminLoginModal');
</script>
@endif
...
<div style="display: none" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-30 font-poppins"
    ...
    <div @click.stop>
        <section class="bg-gray-50 dark:bg-gray-900 relative">
            <button @click="showAdminLoginModal = false; localStorage.setItem('showAdminLoginModal', 'false')" class="absolute top-4 right-4 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 focus:outline-none">
            ...
            <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form x-data="{ isAdminLoadingSignIn: false }" 
                        @submit.prevent="
                        isAdminLoadingSignIn = true;
                        $nextTick(() => $el.submit());" class="space-y-4 md:space-y-6" method="POST" action="{{ route('admin_login') }}">
                        @csrf
                        <div>
                            <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email</label>
                            <input type="email" name="admin_email" id="admin_email" class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="email@example.com">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <a @click.prevent="showAdminForgotPasswordModal = true; localStorage.setItem('showAdminForgotPasswordModal', 'true'); showAdminLoginModal = false; localStorage.setItem('showAdminLoginModal', 'false')"
