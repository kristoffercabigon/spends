@if (session('saveAdminPasswordResetModal'))
    <script>
        console.log("Modal session active, admin_email:", '{{ session('admin_email') }}', "admin_token:", '{{ session('admin_token') }}');
        localStorage.setItem('resetAdminEmail', localStorage.getItem('resetAdminEmail') || '{{ session('admin_email') }}');
        localStorage.setItem('resetAdminToken', localStorage.getItem('resetAdminToken') || '{{ session('admin_token') }}');
        localStorage.setItem('showAdminPasswordResetModal', 'true');
    </script>
@endif

@if (session('removeAdminPasswordResetModal'))
    <script>
        localStorage.removeItem('showAdminPasswordResetModal');
        localStorage.setItem('resetAdminEmail', localStorage.getItem('resetAdminEmail'));
        localStorage.setItem('resetAdminToken', localStorage.getItem('resetAdminToken'));
    </script>
@endif

<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-30 font-poppins"
     x-data="{
         showAdminPasswordResetModal: localStorage.getItem('showAdminPasswordResetModal') === 'true',
         admin_email: localStorage.getItem('resetAdminEmail'), 
         admin_token: localStorage.getItem('resetAdminToken'),
         init() {
             if (this.admin_email) {
                 localStorage.setItem('resetAdminEmail', this.admin_email);
                 document.getElementById('resetAdminEmail').value = this.admin_email;
             }
             if (this.admin_token) {
                 localStorage.setItem('resetAdminToken', this.admin_token);
             }
             localStorage.setItem('showAdminPasswordResetModal', this.showAdminPasswordResetModal);
         }
     }"
     x-show="showAdminPasswordResetModal"
     x-transition:enter="transition-opacity ease-linear duration-300"
     x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
     x-transition:leave="transition-opacity ease-linear duration-300"
     x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
     style="display: none"
     @click.away="showAdminPasswordResetModal = false; localStorage.setItem('showAdminPasswordResetModal', 'false')">
    <div @click.stop>
        <section class="bg-gray-50 relative mx-4 rounded-lg">
            <button @click="showAdminPasswordResetModal = false; localStorage.setItem('showAdminPasswordResetModal', 'false')" 
            ...
            <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <div x-show="admin_email" class="text-sm text-gray-600">
                        Change password for email: <strong x-text="admin_email"></strong>
                    </div>
                    ...
                    <form x-data="{ isLoadingResetPassword: false }"
                        @submit.prevent="
                            ...
                            $nextTick(() => $el.submit());
                        "
                        class="space-y-4 md:space-y-6" method="POST" action="{{ route('admin-reset-password') }}">
                        @csrf
                        @method('PUT')

                        <div class="relative">
                            <label for="admin_password" class="block mb-2 text-sm font-medium text-gray-900">New Password</label>
                        ...
                        <div class="relative">
                            <label for="admin_password_confirmation" class="block mb-2 text-sm font-medium text-gray-900">Confirm Password</label>
...
<script>
function updatePasswordCriteria(admin_password) {
    document.getElementById("minLength").innerHTML = 
        admin_password.length >= 8 ? '<i class="fas fa-check text-green-500"></i> Minimum 8 characters' : 
        '<i class="fas fa-times text-red-500"></i> Minimum 8 characters';
    
    document.getElementById("uppercase").innerHTML = 
        /[A-Z]/.test(admin_password) ? '<i class="fas fa-check text-green-500"></i> At least one uppercase letter' : 
        '<i class="fas fa-times text-red-500"></i> At least one uppercase letter';
    
    document.getElementById("lowercase").innerHTML = 
        /[a-z]/.test(admin_password) ? '<i class="fas fa-check text-green-500"></i> At least one lowercase letter' : 
        '<i class="fas fa-times text-red-500"></i> At least one lowercase letter';
    
    document.getElementById("symbol").innerHTML = 
        /[@$!%*?&]/.test(admin_password) ? '<i class="fas fa-check text-green-500"></i> At least one symbol (@$!%*?&)' : 
...
<script>
    ...
    const admin_email = urlParams.get('admin_email');
    const admin_token = urlParams.get('admin_token');

    if (admin_email) localStorage.setItem('resetAdminEmail', admin_email);
    if (admin_token) localStorage.setItem('resetAdminToken', admin_token);
</script>
