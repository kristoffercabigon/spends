@if (session('clearAdminVerifyCurrentPasswordModal'))
<script>
    localStorage.removeItem('showAdminVerifyCurrentPasswordModal');
</script>
@endif

@if (session('showAdminVerifyCurrentPasswordModal'))
<script>
    localStorage.setItem('showAdminVerifyCurrentPasswordModal', 'true');
</script>
@endif

<div x-show="showAdminVerifyCurrentPasswordModal" style="display: none" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-30 font-poppins"
        x-transition:enter="transition-opacity ease-linear duration-300"
        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
        x-transition:leave="transition-opacity ease-linear duration-300"
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
        @click.away="showAdminVerifyCurrentPasswordModal = false; localStorage.setItem('showAdminVerifyCurrentPasswordModal', 'false')">
    <div @click.stop>
        <section class="bg-gray-50 dark:bg-gray-900 relative">
            <button @click="showAdminVerifyCurrentPasswordModal = false; localStorage.setItem('showAdminVerifyCurrentPasswordModal', 'false')" class="absolute top-4 right-4 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 focus:outline-none">
            ...
            <div class="w-full bg-white rounded-lg shadow dark:border sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form x-data="{ isLoadingChangePassword: false }" 
                            ...
                            class="space-y-4 md:space-y-6" method="POST" action="{{ route('admin-verify-password') }}">
                        @csrf
                        <div class="relative">
                            <label for="admin_current_password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Current Password</label>
                            
                            <input type="password" name="admin_current_password" id="admin_current_password" 
                            ...
                            <button class="absolute inset-y-0 flex items-center justify-center bg-gray-500 text-gray-700 border border-gray-300 rounded-r-md w-10 hover:bg-gray-600 
                                        @error('admin_current_password') h-[46%] mt-[7%] right-[-2px] @else h-[65%] mt-[7%] right-[-2px] @enderror" 
                                    type="button" onclick="togglePassword('admin_current_password', 'togglePasswordIconZ')">
