<div 
    ...
    x-data="{
        showAdminVerificationModal: @json(session('showAdminVerificationModal', false)),
        ...
        resendCode() {
            ...
            fetch('{{ route('admin-resend-code') }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ email: this.admin_email })
            })
        ...
        verifyAdminCode() {
            ...
            const codeInput = document.getElementById('admin_verification_code').value;
            ...
            fetch('{{ route("admin-verify-email-login") }}', {
                method: 'POST',
                body: formData,
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            .then(data => {
                if (data.error) {
                    ...
                    this.showAdminVerificationModal = false;
                    
                    localStorage.setItem('showAdminLoginModal', 'true');
    ...
    x-show="showAdminVerificationModal"
    style="display: none"
    @click.away="showAdminVerificationModal = false; localStorage.setItem('showAdminVerificationModal', 'false'); showAdminLoginModal = false;">
    
    <div @click.stop>
        <section class="bg-gray-50 relative">
            <button @click="showAdminVerificationModal = false; localStorage.setItem('showAdminVerificationModal', 'false')" 
            ...
            <div class="w-full bg-white rounded-lg shadow sm:max-w-md xl:p-0">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <p class="text-sm text-gray-500">
                        ...
                        <strong x-text="admin_email"></strong>
                    </p>

                    <div x-data="{ verifyStatusMessage: '', isLoadingVerify: false }">
                        <form @submit.prevent="verifyAdminCode">
                            @csrf
                            <label for="admin_verification_code" class="block mb-2 text-sm font-medium text-gray-900">
                                Verification Code
                            </label>
                            <input type="text" name="code" id="admin_verification_code" maxlength="6" 
