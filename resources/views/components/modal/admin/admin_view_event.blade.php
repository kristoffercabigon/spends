@if (session('clearAdminViewEventModal'))
<script>
    localStorage.removeItem('showAdminViewEventModal', 'true');
</script>
@endif

<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-30 font-poppins"
     x-show="showAdminViewEventModal"
     x-transition:enter="transition-opacity ease-linear duration-300"
     x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
     x-transition:leave="transition-opacity ease-linear duration-300"
     x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
     style="display: none"
     @click.away="showAdminViewEventModal = false; localStorage.setItem('showAdminViewEventModal', 'false')">
    <div @click.stop>
        <section class="bg-gray-50 relative mx-4 rounded-lg">
            <button @click="showAdminViewEventModal = false; localStorage.setItem('showAdminViewEventModal', 'false')" 
...
<script>
    function loadEventDataForView(id) {
        fetch(`/admin/events-list/getEventDataForView/${id}`)
