@if (session('clearEncoderArchiveBeneficiaryModal'))
<script>
    localStorage.removeItem('showEncoderArchiveBeneficiaryModal', 'true');
</script>
@endif

<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-30 font-poppins"
     x-show="showEncoderArchiveBeneficiaryModal"
     x-transition:enter="transition-opacity ease-linear duration-300"
     x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
     x-transition:leave="transition-opacity ease-linear duration-300"
     x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
     style="display: none"
     @click.away="showEncoderArchiveBeneficiaryModal = false; localStorage.setItem('showEncoderArchiveBeneficiaryModal', 'false')">
    <div @click.stop>
        <section class="bg-gray-50 relative mx-4 rounded-lg">
            <button @click="showEncoderArchiveBeneficiaryModal = false; localStorage.setItem('showEncoderArchiveBeneficiaryModal', 'false')" 
            ...
            <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">
                <div class="p-6 space-y-4 md:space-y-4 sm:p-8">
                    ...
                    <form x-data="{ isEncoderLoadingArchiveBeneficiary: false }"
                        @submit.prevent="
                            isEncoderLoadingArchiveBeneficiary = true;
                            $nextTick(() => $el.submit());
                        "
                        class="space-y-4 md:space-y-6" method="POST" action="{{ route('encoder-submit-archive-beneficiary') }}">
                        @csrf
                        ...
                        <div class="flex justify-between items-center space-x-4">
                            <button type="submit" class="hover:scale-105 transition duration-150 ease-in-out relative w-full text-white bg-red-500 hover:bg-red-600 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                                <span x-show="!isEncoderLoadingArchiveBeneficiary">Yes</span>
                                <span x-show="isEncoderLoadingArchiveBeneficiary" style="display: none;" class="flex items-center justify-center">
                            ...
                            <button type="button" 
                                @click="showEncoderArchiveBeneficiaryModal = false; localStorage.setItem('showEncoderArchiveBeneficiaryModal', 'false')"
...
<script>
    function loadBeneficiaryDataForArchive(id) {
        fetch(`/encoder/beneficiaries/getBeneficiaryDataForArchive/${id}`)
            .then(response => {
                if (!response.ok) {
