@if (session('clearEncoderChangePasswordEmailVerifyModal'))
    <script>
        localStorage.removeItem('showEncoderChangePasswordEmailVerifyModal');
    </script>
@endif

@if (session('showEncoderChangePasswordEmailVerifyModal'))
    <script>
        localStorage.setItem('showEncoderChangePasswordEmailVerifyModal', 'true');
    </script>
@endif

<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-30 font-poppins"
     x-show="showEncoderChangePasswordEmailVerifyModal"
     x-transition:enter="transition-opacity ease-linear duration-300"
     x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
     x-transition:leave="transition-opacity ease-linear duration-300"
     x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
     style="display: none"
     @click.away="showEncoderChangePasswordEmailVerifyModal = false; localStorage.setItem('showEncoderChangePasswordEmailVerifyModal', 'false')">
    <div @click.stop>
        <section class="bg-gray-50 dark:bg-gray-900 relative">
            <button @click="showEncoderChangePasswordEmailVerifyModal = false; localStorage.setItem('showEncoderChangePasswordEmailVerifyModal', 'false')" 
                ...
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form x-data="{ isLoadingChangePassword: false }"
                        @submit.prevent="
                            ...
                            $nextTick(() => $el.submit());
                        "
                        class="space-y-4 md:space-y-6" method="POST" action="{{ route('encoder-verify-change-password-email') }}">
                        @method('POST')
                        @csrf

                        <div>
                            <label for="encoder_verification_code" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Verification Code</label>
                            <input type="hidden" name="encoder_email" value="{{$encoder->encoder_email}}">
                            <input type="text" name="encoder_verification_code" id="encoder_verification_code" maxlength="6"
                                class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Enter the code">

                            @error('encoder_verification_code')
