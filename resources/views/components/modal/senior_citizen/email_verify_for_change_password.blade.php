@if (session('clearChangePasswordEmailVerifyModal'))
    <script>
        localStorage.removeItem('showChangePasswordEmailVerifyModal');
    </script>
@endif

@if (session('showChangePasswordEmailVerifyModal'))
    <script>
        localStorage.setItem('showChangePasswordEmailVerifyModal', 'true');
    </script>
@endif

<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-30 font-poppins"
     x-show="showChangePasswordEmailVerifyModal"
                ...
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form x-data="{ isLoadingChangePassword: false }"
                        @submit.prevent="
                            ...
                            $nextTick(() => $el.submit());
                        "
                        class="space-y-4 md:space-y-6" method="POST" action="{{ route('verify-change-password-email') }}">
                        @method('POST')
                        @csrf
