@if (session('savePasswordResetModal'))
    <script>
        localStorage.setItem('resetEmail', '{{ session('email') }}');
        localStorage.setItem('resetToken', '{{ session('token') }}');
                ...
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    {{-- @if(session('token'))
                        <div class="text-sm text-gray-600 dark:text-gray-400">
                            Your token for password reset is: <strong>{{ session('token') }}</strong>
                        </div>
                    @endif --}}
                    ...
                    @if(session('email'))
                        <div class="text-sm text-gray-600 dark:text-gray-400">
                            Change password for email: <strong>{{ session('email') }}</strong>
                        </div>
                    @endif
