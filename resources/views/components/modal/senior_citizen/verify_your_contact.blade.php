<div 
    ...
    x-data="{
        showVerificationContactModal: @json(session('showVerificationContactModal', false)),
        ...
        resendCode() {
            ...
            fetch('{{ route('resend-code') }}', {
                ...
                body: JSON.stringify({ contact_no: this.contact_no })
            })
            .then(response => response.json())
            .then(data => {
                this.isLoadingResend = false;
                if (data.message) {
                    this.statusMessage = 'A new verification code has been sent to your contact number.';
                } else if (data.error) {
        ...
        verifyCode() {
            ...
            const codeInput = document.getElementById('verification_code').value;
            ...
            .then(data => {
                if (data.error) {
                    ...
                    this.showVerificationContactModal = false;
    ...
    x-show="showVerificationContactModal"
    x-transition:enter="transition-opacity ease-linear duration-300"
     x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
     x-transition:leave="transition-opacity ease-linear duration-300"
     x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
    style="display: none"
    @click.away="showVerificationContactModal = false; localStorage.setItem('showVerificationContactModal', 'false'); showLoginModal = false;" class="w-full bg-white mx-4 rounded-lg shadow sm:max-w-md xl:p-0">
    
    <div @click.stop>
        <section class="bg-gray-50 mx-4 relative rounded-lg">
            <button @click="showVerificationContactModal = false; localStorage.setItem('showVerificationContactModal', 'false')" 
            ...
            <div class="w-full bg-white rounded-lg shadow sm:max-w-md xl:p-0">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <p class="text-sm text-gray-500">
                        Please enter the 6-digit verification code sent to your contact number.
                        <strong x-text="contact_no"></strong>
                    ...
                    <div x-data="{ verifyStatusMessage: '', isLoadingVerify: false }">
                        <form @submit.prevent="verifyCode">
                            @csrf
                            <label for="verification_code" class="block mb-2 text-sm font-medium text-gray-900">
                                Verification Code
                            </label>
