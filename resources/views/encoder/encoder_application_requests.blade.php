...
<section x-data="{ showEncoderArchiveSeniorApplicationModal: localStorage.getItem('showEncoderArchiveSeniorApplicationModal') === 'true',
showEncoderRestoreApplicationModal: localStorage.getItem('showEncoderRestoreApplicationModal') === 'true' }" class="bg-cover bg-center bg-no-repeat min-h-screen" class="bg-cover bg-center bg-no-repeat min-h-screen" style="background-image: url('{{ asset('images/background2.png') }}'); background-attachment: fixed;">
    <ul class="circles">
        <li></li>
        <li></li>
        <li></li>
...
<div x-show="showEncoderArchiveSeniorApplicationModal" @click.away="showEncoderArchiveSeniorApplicationModal = false" class="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
    @include('components.modal.encoder.encoder_archive_application')
</div>
<div x-show="showEncoderRestoreApplicationModal" @click.away="showEncoderRestoreApplicationModal = false" class="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
    @include('components.modal.encoder.encoder_restore_application')
</div>
</section>
...
<script>
    ...
    function updateTable(page) {
        ...
        fetch('/encoder/application-requests/filter-application-requests?page=' + page, {
            method: 'POST',
            headers: {
        ...
        data.forEach((senior, index) => {
            ...
            const actionButton = senior.is_application_archived === 0 ? `
                <a 
                    ...
                    @click="showEncoderArchiveSeniorApplicationModal = true; 
                    localStorage.setItem('showEncoderArchiveSeniorApplicationModal', 'true');
                ...
                <a 
                    ...
                    @click="showEncoderRestoreApplicationModal = true; 
                    localStorage.setItem('showEncoderRestoreApplicationModal', 'true');
            ...
            const row = `
                <tr class="${index % 2 === 0 ? 'bg-[#ffece5]' : 'bg-[#ffc8b3]'}">
                    ...
                    <td class="px-4 py-2 flex justify-start items-center whitespace-nowrap w-[150px] shrink-0">
                        <a href="/encoder/application-requests/view-senior-profile/${senior.id}" class="bg-blue-500 animate-pop hover:bg-blue-600 rounded-md p-2 cursor-pointer">
