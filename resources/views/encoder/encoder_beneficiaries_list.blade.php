...
<section x-data="{ showEncoderArchiveBeneficiaryModal: localStorage.getItem('showEncoderArchiveBeneficiaryModal') === 'true',
showEncoderRestoreBeneficiaryModal: localStorage.getItem('showEncoderRestoreBeneficiaryModal') === 'true' }" class="bg-cover bg-center bg-no-repeat min-h-screen" style="background-image: url('{{ asset('images/background2.png') }}'); background-attachment: fixed;">
    <ul class="circles">
        <li></li>
        <li></li>
        <li></li>
                ...
                <div class="px-6 py-4 lg:px-12">
                    ...
                    <div class="grid mb-4 grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="flex relative justify-start mb-4 md:justify-end">
                                    ...
                                    <ul class="p-3 space-y-3 text-sm text-gray-700" aria-labelledby="dropdownCheckboxButton">
                                        ...
                                        <li>
                                            <div class="flex items-center">
                                                ...
                                                <label for="checkbox-item-{{ $status->id }}" class="ms-2 text-sm font-medium text-gray-900">{{ $status->senior_account_status }}</label>
                                            </div>
                                        </li>
                                        @endforeach
                                    </ul>
...
<div x-show="showEncoderArchiveBeneficiaryModal" @click.away="showEncoderArchiveBeneficiaryModal = false" class="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
    @include('components.modal.encoder.encoder_archive_beneficiary')
</div>
<div x-show="showEncoderRestoreBeneficiaryModal" @click.away="showEncoderRestoreBeneficiaryModal = false" class="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
    @include('components.modal.encoder.encoder_restore_beneficiary')
</div>
</section>
...
<script>
    ...
    const startPicker = flatpickr(startInput, {
        ...
        onClose: function (selectedDates) {
            const beneficiaryStartDate = selectedDates[0];
            localStorage.setItem('beneficiaryStartDate', beneficiaryStartDate ? beneficiaryStartDate.toLocaleDateString('en-CA') : '');
            updateTable(1);
        }
    });
    ...
    const endPicker = flatpickr(endInput, {
        ...
        onClose: function (selectedDates) {
            const beneficiaryEndDate = selectedDates[0];
            localStorage.setItem('beneficiaryEndDate', beneficiaryEndDate ? beneficiaryEndDate.toLocaleDateString('en-CA') : '');
            updateTable(1);
        }
    });
    ...
    document.getElementById("clear-start").addEventListener("click", function () {
        startInput.value = '';
        localStorage.removeItem('beneficiaryStartDate');
        startPicker.clear();
        updateTable(1);
    });
    ...
    document.getElementById("clear-end").addEventListener("click", function () {
        endInput.value = '';
        localStorage.removeItem('beneficiaryEndDate');
        endPicker.clear();
        updateTable(1);
    });

    barangayDropdown.addEventListener("change", function () {
        const beneficiaryBarangayId = this.value;
        localStorage.setItem('beneficiaryBarangayId', beneficiaryBarangayId);
        updateTable(1);
    });
    ...
    function updateTable(page) {
        const beneficiaryBarangayId = barangayDropdown.value === 'all' ? null : barangayDropdown.value;
        ...
        fetch('/encoder/beneficiaries/filter-beneficiaries?page=' + page, {
            ...
            body: JSON.stringify({
                ...
                status_ids: beneficiarySelectedStatuses,
                search_query: beneficiarySearchQuery,
                order: beneficiaryOrder,
            }),
        })
        ...
        data.forEach((senior, index) => {
            ...
            const actionButton = senior.is_beneficiary_archived === 0 ? `
                <a 
                    ...
                    @click="showEncoderArchiveBeneficiaryModal = true; 
                    localStorage.setItem('showEncoderArchiveBeneficiaryModal', 'true');
                ...
                <a 
                    ...
                    @click="showEncoderRestoreBeneficiaryModal = true; 
                    localStorage.setItem('showEncoderRestoreBeneficiaryModal', 'true');
